...
**6 AI features, each with an eval suite · 443 tests + a 6-suite AI eval harness · ~20k lines of strict TypeScript (app, tests, e2e, evals, scripts)**
...
- **Accessibility is a gate, not an aspiration.** [`e2e/a11y.spec.ts`](e2e/a11y.spec.ts) runs axe over all 15 screens — every authenticated page, every page you meet before you have an account, and each of the desk's three tabs, which stay mounted and hidden so a page-level scan would only ever measure the first — in **both themes**, failing on any serious or critical violation, plus the keyboard paths through the tabs and the command palette. Each check asserts the page actually rendered first, because a screen that 404s scans clean and a gate that only counts violations would report green for something it never reached. It keeps earning its place: it caught a candidate palette whose muted ink measured **4.13:1** against the page — under AA, and invisible to the eye — a posting pane that scrolled under a mouse but could not be reached by keyboard at all, and, the first time the sweep was widened to every screen, the same keyboard trap a second time on the extracted-résumé pane.
...
- **Bullet tailoring** — LLM-judge relevance **4.83/5**, grounding **4.83/5**, formatting **5/5**, **zero hallucinations** across 6 rewrites. This is the suite that changed the app: it caught the lite model inventing specifics the source never claimed (grounding 3.83/5, fabricating in **half** its outputs), so tailoring alone was routed to the stronger **`gemini-3.5-flash`** — **3.83 → 4.83 grounding, 50% → 0% hallucination**. Then the same suite was turned on that decision: re-running tailoring against the *current* lite model scored 4.4/5 grounding and a 20% hallucination rate against a 20% ceiling, with the miss on the item holding the least source experience — the exact place a weaker model fills a gap by inventing. The exception is kept because it was measured twice, not because it was decided once.
...
443 tests across two Vitest projects, plus three Playwright suites (44 browser tests including the sign-in setup) — every one of them gating each push and pull request:
...
- **Integration (10 of the 443)** — run against a real Postgres, and skip locally when no database is reachable. CI always runs them against a `pgvector/pgvector` service container, so the raw SQL the mocked unit tests can't reach stays covered: the rate limiter's atomic upsert, and the predicate deciding whether a resume holds readable text.
